...
void printDataTime(){//used to print system time
    ...
    // Nama oli alkuperaisessa koodissa
    //lcdPosition(lcdhd,0,1);// set the LCD cursor position to (0,1)
    //lcdPrintf(lcdhd,"Time:%2d:%2d:%2d",timeinfo->tm_hour,timeinfo->tm_min,timeinfo->tm_sec); //Display system time on LCD

    if(timeinfo->tm_hour < 10)
    {
    lcdPosition(lcdhd, 0, 1);
    lcdPrintf(lcdhd, "Time:0%d:", timeinfo->tm_hour);
    } else {
    lcdPosition(lcdhd, 0, 1);
    lcdPrintf(lcdhd, "Time:%2d:", timeinfo->tm_hour);
    }

    if(timeinfo->tm_min < 10) {
    lcdPosition(lcdhd, 8, 1);
    lcdPrintf(lcdhd, "0%d:", timeinfo->tm_min);
    } else {
    lcdPosition(lcdhd, 8, 1);
    lcdPrintf(lcdhd, "%2d:", timeinfo->tm_min);
    }

    if(timeinfo->tm_sec < 10) {
    lcdPosition(lcdhd, 11, 1);
    lcdPrintf(lcdhd, "0%d", timeinfo->tm_sec);
    } else {
    lcdPosition(lcdhd, 11, 1);
    lcdPrintf(lcdhd, "%2d", timeinfo->tm_sec);
    }
